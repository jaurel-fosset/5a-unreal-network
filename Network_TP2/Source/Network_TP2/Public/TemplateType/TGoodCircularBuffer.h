// Copyright Epic Games, Inc. All Rights Reserved.

#pragma once

#include <rapidjson/rapidjson.h>

#include "CoreTypes.h"
#include "Algo/Count.h"
#include "Misc/AssertionMacros.h"
#include "Containers/Array.h"
#include "DataWrappers/ChaosVDParticleDataWrapper.h"
#include "Math/UnrealMathUtility.h"

/**
 * Template for circular buffers.
 *
 * The size of the buffer is rounded up to the next power of two in order speed up indexing
 * operations using a simple bit mask instead of the commonly used modulus operator that may
 * be slow on some platforms.
 */
template<typename InElementType> class TGoodCircularBuffer
{
public:
	using ElementType = InElementType;

	[[nodiscard]] explicit TGoodCircularBuffer()
	{
		Elements = TArray<ElementType>();
		IndexMask = Elements.Num() - 1;
	}
	
	/**
	 * Creates and initializes a new instance of the TCircularBuffer class.
	 *
	 * @param Capacity The number of elements that the buffer can store (will be rounded up to the next power of 2).
	 */
	[[nodiscard]] explicit TGoodCircularBuffer(uint32 Capacity)
	{
		checkSlow(Capacity > 0);
		checkSlow(Capacity <= 0xffffffffU);

		Elements.AddZeroed(FMath::RoundUpToPowerOfTwo(Capacity));
		IndexMask = Elements.Num() - 1;
	}

	/**
	 * Creates and initializes a new instance of the TCircularBuffer class.
	 *
	 * @param Capacity The number of elements that the buffer can store (will be rounded up to the next power of 2).
	 * @param InitialValue The initial value for the buffer's elements.
	 */
	[[nodiscard]] TGoodCircularBuffer(uint32 Capacity, const ElementType& InitialValue)
	{
		checkSlow(Capacity <= 0xffffffffU);

		Elements.Init(InitialValue, FMath::RoundUpToPowerOfTwo(Capacity));
		IndexMask = Elements.Num() - 1;
	}

public:

	/**
	 * Returns the mutable element at the specified index.
	 *
	 * @param Index The index of the element to return.
	 */
	[[nodiscard]] FORCEINLINE ElementType& operator[](uint32 Index)
	{
		return Elements[Index & IndexMask];
	}

	/**
	 * Returns the immutable element at the specified index.
	 *
	 * @param Index The index of the element to return.
	 */
	[[nodiscard]] FORCEINLINE const ElementType& operator[](uint32 Index) const
	{
		return Elements[Index & IndexMask];
	}
	
public:

	/**
	 * Returns the number of elements that the buffer can hold.
	 *
	 * @return Buffer capacity.
	 */
	[[nodiscard]] FORCEINLINE uint32 Capacity() const
	{
		return Elements.Num();
	}

	/**
	 * Calculates the index that follows the given index.
	 *
	 * @param CurrentIndex The current index.
	 * @return The next index.
	 */
	[[nodiscard]] FORCEINLINE uint32 GetNextIndex(uint32 CurrentIndex) const
	{
		return ((CurrentIndex + 1) & IndexMask);
	}

	/**
	 * Calculates the index previous to the given index.
	 *
	 * @param CurrentIndex The current index.
	 * @return The previous index.
	 */
	[[nodiscard]] FORCEINLINE uint32 GetPreviousIndex(uint32 CurrentIndex) const
	{
		return ((CurrentIndex - 1) & IndexMask);
	}

public:

	TArray<ElementType> GetRaw() const
	{
		return Elements;
	}

	void SetNum(const uint32 NewCapacity)
	{
		checkSlow(NewCapacity > 0);
		checkSlow(NewCapacity <= 0xffffffffU);

		Elements.SetNumZeroed(FMath::RoundUpToPowerOfTwo(NewCapacity));
		
		IndexMask = Elements.Num() - 1;
	}

	template <typename Predicate>
	uint32 FindLastByPredicate(const uint32 CurrentIndex, Predicate Pred) const
	{
		checkSlow(CurrentIndex < IndexMask + 1);
		
		for (uint32 Index = CurrentIndex; Index != GetNextIndex(CurrentIndex); Index = GetPreviousIndex(Index))
		{
			if (::Invoke(Pred, Elements[Index]))
				return Index;
		}
		
		return -1;
	}

	TArray<ElementType>::RangedForConstIteratorType begin() const { return Elements.begin(); }
	TArray<ElementType>::RangedForConstIteratorType end()   const { return Elements.end(); }

private:
	/** Holds the mask for indexing the buffer's elements. */
	uint32 IndexMask;

	/** Holds the buffer's elements. */
	TArray<ElementType> Elements;
};